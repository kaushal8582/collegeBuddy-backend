...
const uploadPYQ = asyncHandler(async (req, res) => {
  ...
  if (
    [universityName, semester, year, courseName, questionLink].some(
      (item) => item?.trim() === ""
    )
  ) {
  ...
  if (!previousYearQuestion) {
    throw new ApiError(
      500,
      "Something went wrong while uploading a previous year question"
    );
  }
...
const findPYQ = asyncHandler(async (req, res) => {
  const { universityName, semester, courseName, year } = req.body;

  if (!universityName && !semester && !courseName && !year) {
    throw new ApiError(400, "Minimun one field are required");
  }
  ...
  const questionPaper = await Pyq.find(query);
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, "Question find successfully ", questionPaper));
});
...
const downloadPYQ = asyncHandler(async (req, res) => {
  ...
  return res
    ...
    .json(
      new ApiResponse(
        200,
        "PYQ find successfully ",
        preiousQuestion.questionLink
      )
    );
});
...
const getAllPyq = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, "Successfully find all question ", allPyq));
});
...
const deletePyq = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, "PYQ delete successfully", {}));
});

export { uploadPYQ, findPYQ, downloadPYQ, getAllPyq, deletePyq };
